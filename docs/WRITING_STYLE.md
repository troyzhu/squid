...
- **Intuition before formalism, and motivate the object.** A load-bearing concept leads with a concrete analogy or a mental handle before the precise statement — not the formula first (a correct result stated cold reads flat). A load-bearing formal object is motivated before its closed form is given — shown forced by a no-arbitrage / optimality / symmetry argument, and why this functional form is the natural one — not asserted. Connect to the reader's domain where the structure genuinely matches (for an ML reader: Bellman/MDP value functions, two-player games, importance weights).
